...
func apply_effect(target: Node,caster: Node): # Rufe die Basis-Implementierung auf
	...
	if target is Player: # Stelle sicher, dass es der Spieler ist
		...
		if target.has_signal("was_attacked"):
			target.was_attacked.connect(on_player_attacked)
...
func remove_effect(target: Node,caster: Node):
	...
	if target.has_signal("was_attacked") and target.was_attacked.is_connected(on_player_attacked):
		target.was_attacked.disconnect(on_player_attacked)
...
func on_player_attacked(attacker: Node, damage_taken: float):
